...
// Tambah trainer ke window.NPCS dan zone
if (window.NPCS) Object.assign(window.NPCS, SKILL_TRAINERS);
...
function chooseMasterClass(masterClassId) {
  ...
  const lv = window.selChar.level || 1;
  if (lv < mc.reqLv) {
    if(typeof addChat==='function')addChat('','⚠️ Level tidak mencukupi! Perlukan Lv.'+mc.reqLv,'system');
    return;
  }
...
// openNPC hook — deferred
function _hookSKMOpenNPC() {
  if (typeof openNPC !== 'function') { setTimeout(_hookSKMOpenNPC, 200); return; }
  const _origOpenNPC = openNPC;
  openNPC = window.openNPC = function(npcId) {
...
setTimeout(_hookSKMOpenNPC, 400);
