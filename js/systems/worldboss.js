...
window.WorldBoss = {
  ...
  spawn() {
    ...
    if (el) {
      ...
      const nm = el.querySelector('#worldBossName');
      if (nm) nm.textContent = this.type === 'world_dragon' ? '🐉 Naga Dunia' : '👿 World Boss';
      const rw = el.querySelector('#worldBossReward');
      if (rw) rw.textContent = '🏆 Top damage dapat reward Legendary!';
    }
  },
  ...
  takeDamage(dmg, player) {
    ...
    const bar = document.getElementById('worldBossHpBar') || document.getElementById('worldBossBar');
    if (bar) bar.style.width = pct + '%';
    const lbl = document.getElementById('worldBossHpText') || document.getElementById('worldBossHp');
    if (lbl) lbl.textContent = `${this.hp.toLocaleString()} / ${this.maxHp.toLocaleString()} HP`;
