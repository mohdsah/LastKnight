...
// Sanitize HTML untuk elak XSS — escape aksara berbahaya
...
async function searchEditPlayer(){
  ...
  res.innerHTML=data.map(p=>`<div onclick='loadEditPlayer(${JSON.stringify(p).replace(/'/g,"&#39;")})'
    ...
    ${esc(p.char_name)} — Lv.${p.level} ${p.faction==='elmorad'?'🌟':'🔥'}</div>`).join('');
}
function loadEditPlayer(p){
  try{editingChar=typeof p==='string'?JSON.parse(p):p;}catch(e){editingChar={};console.error('GM parse error:',e);}
...
function renderItemSelector(cat){
  ...
  g.innerHTML=(ITEM_CATS[cat]||[]).map(id=>`<div class="item-cell${selectedItemId===id?' selected':''}" onclick="selectItem('${esc(id)}')"><div class="item-icon">${ITEM_ICONS[id]||'📦'}</div><div class="item-name">${esc(id)}</div></div>`).join('');
}
...
async function searchGivePlayer(){
  ...
  res.innerHTML=`✅ ${esc(data[0].char_name)} — Lv.${data[0].level} ${data[0].faction==='elmorad'?'🌟':'🔥'}`;
}
...
async function loadTopupRequests(){
  ...
  tb.innerHTML=data.map(r=>`<tr><td style="color:var(--gold)">${esc(r.char_name||'?')}</td><td>${esc(r.full_name||'-')}</td><td style="color:var(--gold)">RM ${r.amount_rm||0}</td><td style="color:var(--purple)">💎 ${r.points_req||0}</td><td style="font-size:.6rem;color:var(--muted)">${esc(r.ref_no||'-')}</td><td style="font-size:.6rem;color:var(--muted)">${new Date(r.created_at).toLocaleDateString('ms-MY')}</td><td><button class="btn btn-green btn-xs" onclick="approveTopup('${r.id}','${esc(r.char_name)}',${r.points_req||0})">✅</button> <button class="btn btn-red btn-xs" onclick="rejectTopup('${r.id}')">❌</button></td></tr>`).join('');
}
...
async function loadBanList(){
  ...
  tb.innerHTML=data.map(b=>`<tr><td style="color:#e84040">${esc(b.char_name)}</td><td>${esc(b.reason||'-')}</td><td style="font-size:.6rem;color:var(--muted)">${new Date(b.created_at).toLocaleDateString('ms-MY')}</td><td><button class="btn btn-green btn-xs" onclick="doUnban('${esc(b.char_name)}')">Unban</button></td></tr>`).join('');
}
...
function renderPlayerTable(){
  ...
  tb.innerHTML=filteredPlayers.map((p,i)=>{
    ...
    return`<tr>
      <td><span class="pl-faction-${isEl?'el':'kr'}">${esc(p.char_name||'?')}</span></td>
      ...
      <td><div class="pl-actions">
        <button class="btn btn-gold btn-xs" onclick="quickEditPlayer('${esc(p.char_name)}')">✏️</button>
        <button class="btn btn-green btn-xs" onclick="quickInv('${esc(p.char_name)}')">🎒</button>
        <button class="btn btn-blue btn-xs" onclick="quickGiveItem('${esc(p.char_name)}')">🎁</button>
        <button class="btn btn-red btn-xs" onclick="quickBan('${esc(p.char_name)}')">🚫</button>
      </div></td>
    </tr>`;
...
async function loadDashboard(){
  try{
    ...
    if(rb)rb.innerHTML=(topPlayers||[]).map((p,i)=>{
      const isEl=p.faction==='elmorad'||p.faction==='cahaya';
      return`<tr><td style="color:var(--muted)">${i+1}</td><td><span class="pl-faction-${isEl?'el':'kr'}">${esc(p.char_name)}</span></td><td>${p.level||1}</td><td>${isEl?'🌟':'🔥'}</td><td><button class="btn btn-gold btn-xs" onclick="quickEditPlayer('${esc(p.char_name)}')">Edit</button></td></tr>`;
    }).join('');
    // Recent log
    const rl=$el('dashRecentLog');
    if(rl)rl.innerHTML=(recentLog||[]).map(l=>`<tr><td class="log-action">${esc(l.action)}</td><td style="color:var(--gold);font-family:Share Tech Mono,monospace;font-size:.65rem">${esc(l.gm_name)}</td><td class="log-target">${esc(l.target||'-')}</td><td class="log-detail">${esc(l.detail||'-')}</td><td class="log-time">${new Date(l.created_at).toLocaleString('ms-MY')}</td></tr>`).join('');
...
function filterLog(){
  ...
  tb.innerHTML=filtered.map(l=>`<tr><td class="log-action">${esc(l.action||'?')}</td><td style="color:var(--gold);font-family:Share Tech Mono,monospace;font-size:.65rem">${esc(l.gm_name)}</td><td class="log-target">${esc(l.target||'-')}</td><td class="log-detail">${esc(l.detail||'-')}</td><td class="log-time">${new Date(l.created_at).toLocaleString('ms-MY')}</td></tr>`).join('')||'<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:16px">Tiada log</td></tr>';
}
