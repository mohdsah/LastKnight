...
function showDropNotif(itemId, rarity) {
  const item = window.ITEM_DB[itemId]; if (!item || !G.pl) return;
  const col = window.RARITY_COLOR[rarity] || '#aaa';
  G.fts.push(new FT(G.pl.x, G.pl.y - 45, '+ ' + item.name, col, 13));
  // Audio SFX
...
function openStatAlloc() {
  ...
  if (pts <= 0) { addChat('','No stat points available!','system'); return; }
  const box = document.getElementById('statAllocPanel');
  if (!box) return;
...
function initCZ() {
  // Deep clone CZ data
  czData = JSON.parse(JSON.stringify(window.CZ_ZONE));
  czTimer  = CZ_DURATION;
  czActive = true;
...
function drawCZMap() {
  ...
  // ── Lukis Colonies ────────────────────────────────
  for (const col of czData.colonies) {
    const colCol = col.owner === 'elmorad' ? '#c9a84c' : col.owner === 'karus' ? '#cc3333' : '#666666';
...
// ── Panel CZ ─────────────────────────────────────────
function openCZPanel() {
  const p = document.getElementById('czPanel'); if (!p) return;
