...
async function goMenu(){
  ...
  // Avatar — race icon
  const av = E('mAvatar'); if(av) av.textContent = race.icon;
...
async function loadLB(){
  const lb=E('lbList');
  if(window.offlineMode||!SB){lb.innerHTML='<div style="color:var(--muted);text-align:center;font-size:.7rem;padding:7px">Offline mode</div>';return}
  try{
...
function updHUD(){
  ...
  E('hpB').style.width=(p.hp/p.maxHp*100)+'%';
  E('hpV').textContent=Math.ceil(p.hp)+'/'+Math.ceil(p.maxHp);
  E('mpB').style.width=(p.mp/p.maxMp*100)+'%';
  E('mpV').textContent=Math.ceil(p.mp)+'/'+Math.ceil(p.maxMp);
  const lv=window.selChar?.level||1,xp=window.selChar?.xp||0,need=lv*100;
  E('xpB').style.width=(xp/need*100)+'%';
  E('xpV').textContent=xp+'/'+need;
  E('lvB').textContent='LV '+lv;
  E('wN').textContent=G.wave;
  E('scN').textContent=G.score.toLocaleString();
  E('klN').textContent='Kills: '+G.kills;
  // Skill CD
...
function updMinimap(){
  ...
  // Other players
  for(const op of Object.values(window.opMap||{})){
...
function startNationWar(){
  nwActive=true;nwTimer=window.NW_CONFIG.duration;nwScore={elmorad:0,karus:0};
...
function endNationWar(){
  ...
  if(window.selChar){
    ...
    const gold=isWinner?window.NW_CONFIG.rewardGold.winner:window.NW_CONFIG.rewardGold.loser;
    const xp=isWinner?window.NW_CONFIG.rewardXP.winner:window.NW_CONFIG.rewardXP.loser;
...
function openParty(){E('partyPanel').classList.remove('off');renderPartyPanel()}
function renderPartyPanel(){
  ...
  let html=`<div style="font-family:'Cinzel',serif;font-size:.65rem;color:rgba(201,168,76,.5);text-transform:uppercase;margin-bottom:8px">
    Party Members (${myParty.members.length+1}/${window.PARTY_CONFIG.maxMembers})</div>`;
  // Leader (self)
  if(window.selChar){
  ...
  // Invite code
  const code=window.curAccount?.uid?.slice(-6).toUpperCase()||'??????';
...
let pkMode=false;
function togglePK(){
  const zone=window.ZONES[G.currentZone||'moradon'];
...
function healAtInn(){
  ...
  window.selChar.gold-=50;G.pl.hp=G.pl.maxHp;G.pl.mp=G.pl.maxMp;
  addChat('','Rested at inn. HP/MP fully restored!','system');closeNPC();saveProgress();
}
...
function renderBag(){
  ...
  for(let i=0;i<20;i++){
    if(i<items.length){
      ...
      html+=`<div class="inv-slot has-item" onclick="selBagItem('${iid}')" title="${item.name}">
...
function selEquipSlot(slot){const iid=window.selChar.equipment?.[slot];if(iid)selBagItem(iid)}
...
function equipItem(iid){
  ...
  renderBag();saveProgress();
}
function unequipItem(iid){
  if(!window.selChar.equipment)return;
  for(const s in window.selChar.equipment){if(window.selChar.equipment[s]===iid){window.selChar.equipment[s]=null;window.selChar.inventory[iid]=(window.selChar.inventory[iid]||0)+1;if(G.pl)G.pl.applyChar(window.selChar);renderEquipSlots();saveProgress();return}}
}
function usePotion(iid){
  const item=window.ITEM_DB[iid];if(!item||!G.pl)return;
...
function buyItem(iid){
  ...
  renderShopTab(curShopTab);saveProgress();
}

// ── DUNGEON ───────────────────────────────────────────────────────
function openDungeon(){E('dungeonPanel').classList.remove('off');renderDungeons()}
...
// ── NATION WAR PANEL ──────────────────────────────────────────────
function openNWPanel(){E('nwPanel').classList.remove('off');renderNWPanel()}
...
function openTrade(){E('tradePanel').classList.remove('off');renderTrade()}
...
function showLvNotif(){
  const el=E('lvnotif');el.textContent='⬆ LEVEL UP! '+window.selChar?.level;el.classList.add('sh');
  for(let i=0;i<28;i++){const a=(i/28)*Math.PI*2,sp=90+Math.random()*140;G.pts.push(new Pt(G.pl.x,G.pl.y,Math.cos(a)*sp,Math.sin(a)*sp,'#ffcc44',5,.75))}
...
function startGame(){
  ...
  if(!window.offlineMode&&SB)sc('opill','on');
  ...
  for(const k in window.opMap)delete window.opMap[k];G.init();initRT();
  if(window.selChar)showZoneBanner(G.currentZone||'moradon');
}
function useQuickPotion(){
  if(!G.pl||!window.selChar)return;
