...
function renderCharSlots() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (ch) {
      ...
      div.innerHTML = `
        <div class="cs-avatar">${race.icon}</div>
...
function cycleFace(dir) {
  const icons = window.FACE_ICONS[pendingFaction]||window.FACE_ICONS.elmorad;
...
async function createCharacter() {
  ...
  if (!offlineMode && SB) {
    ...
    c.inventory  = JSON.parse(c.inventory);
    c.equipment  = JSON.parse(c.equipment);
    c.skill_tree = JSON.parse(c.skill_tree);
    curChars.push(c);
  }
...
// ── Alias untuk init.js ───────────────────────────────
if (typeof doLogin==='function') window._doLogin = doLogin;
if (typeof doRegister==='function') window._doRegister = doRegister;
if (typeof cancelLogin==='function') window._cancelLogin = cancelLogin;
if (typeof skipConfig==='function') window._skipConfig = skipConfig;
if (typeof selectFaction==='function') window._selectFaction = selectFaction;
if (typeof confirmFaction==='function') window._confirmFaction = confirmFaction;
if (typeof backToFaction==='function') window._backToFaction = backToFaction;
if (typeof createCharacter==='function') window._createCharacter = createCharacter;
if (typeof enterWorld==='function') window._enterWorld = enterWorld;
if (typeof deleteChar==='function') window._deleteChar = deleteChar;
if (typeof addStat==='function') window._addStat = addStat;
if (typeof cycleFace==='function') window._cycleFace = cycleFace;
if (typeof doLogout==='function') window._doLogout_real = doLogout;
