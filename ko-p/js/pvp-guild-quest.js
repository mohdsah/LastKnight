...
function checkPvPHit(playerX, playerY, atkRange, atkDir) {
  ...
  for (const [id, op] of Object.entries(window.opMap)) {
    ...
    const dmg = Math.floor((G.pl?.atk || 20) * (0.8 + Math.random() * 0.4));
    ...
    // Floating text
    G.fts?.push(new FT(op.x, op.y - 20, '⚔ -' + dmg, '#ff4444', 14));
...
function handlePvPHit(payload) {
  ...
  if (G.pl) {
    G.pl.hurt2(dmg);
    G.fts?.push(new FT(G.pl.x, G.pl.y - 20, '⚔ -' + dmg + ' [PvP]', '#ff2222', 14));
    if (G.pl.hp <= 0) {
      PVP.addDeath(payload.fromName || '?');
      // Respawn selepas 3 saat di Moradon
      setTimeout(() => {
        G.pl.hp = G.pl.maxHp * 0.3;
        G.pl.x  = window.ZONES.moradon.spawnX;
        G.pl.y  = window.ZONES.moradon.spawnY;
        addChat('', '💀 Anda gugur! Respawn di Moradon.', 'system');
      }, 3000);
    }
  }
}
...
// Hook ke Player.doAtk untuk check PvP
...
async function renderMarketMyListings(container) {
  if (!SB || window.offlineMode) { container.innerHTML = '<div style="color:var(--muted);text-align:center;padding:20px">Perlu sambungan internet</div>'; return; }
  const { data } = await SB.from('kn_market').select('*').eq('seller_name', window.selChar?.char_name).order('created_at',{ascending:false});
...
async function leaveGuild() {
  ...
  window.selChar.guild_name = null; saveProgress(); myGuild = null; loadGuildData();
}
...
function loadQuestProgress() {
  if (!window.selChar?.skill_tree) return;
  try {
    const raw = window.selChar.skill_tree['_quests'];
    questProgress = raw ? JSON.parse(raw) : {};
  } catch { questProgress = {}; }
}
...
function saveQuestProgress() {
  ...
  window.selChar.skill_tree['_quests'] = JSON.stringify(questProgress);
  saveProgress();
}
...
function tickCastleSiege(dt) {
  ...
  if (G.pl && pkMode) {
    ...
    Object.values(CASTLE_ZONES).forEach(c => {
      ...
      const dist = Math.hypot(G.pl.x - c.x, G.pl.y - c.y);
      if (dist < 80) {
        c.hp -= 5;
...
const _origG_endGame = G.endGame?.bind(G);

// Hook monster kill reward
const _origFarmingInitZone = farmingInitZone;
farmingInitZone = function(zoneId) {
  _origFarmingInitZone(zoneId);
  // Load quest progress bila masuk zone
  loadQuestProgress();
};
...
window.onEnemyKilled = function(enemyType, enemyX, enemyY) {
  ...
  // PvP kill check
  if (G.currentZone === 'cz' && siegeActive) {
    const myFac = window.selChar?.faction === 'cahaya' ? 'elmorad' : window.selChar?.faction;
    if (siegeData) siegeData[myFac==='elmorad'?'elScore':'krScore'] += 5;
  }
};

// Hook enterWorld untuk load semua data
