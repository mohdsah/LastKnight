...
function tickInnBuffs(dt){
  ...
  if(changed&&G.pl)G.pl.applyChar(window.selChar);
  updateInnBuffHUD();
}
...
function applyInnBuff(effect){
  if(effect.healFull&&G.pl){G.pl.hp=G.pl.maxHp;G.pl.mp=G.pl.maxMp;G.fts?.push(new FT(G.pl.x,G.pl.y-50,'✨ PULIH PENUH!','#40e860',16))}
  ...
  if(effect.atkBuff) {innBuffs.atk={active:true,multiplier:effect.atkBuff,remaining:effect.buffDur};if(G.pl)G.pl.atk*=effect.atkBuff}
  if(effect.defBuff) {innBuffs.def={active:true,multiplier:effect.defBuff,remaining:effect.buffDur};if(G.pl)G.pl.def*=effect.defBuff}
  if(effect.spdBuff) {innBuffs.spd={active:true,multiplier:effect.spdBuff,remaining:effect.buffDur};if(G.pl)G.pl.speed*=effect.spdBuff}
  if(effect.save)    {saveProgress();addChat('','💾 Progress disimpan!','system')}
...
async function doUpgrade(safe){
  ...
  if(safe||roll<rate.success){
    ...
    // Protected — item tidak hancur tapi protection hilang
    result='protected';
...
async function doReversal(){
  ...
  saveProgress();isUpgrading=false;
...
async function doCraft(recipeId){
  const r=CRAFT_RECIPES.find(x=>x.id===recipeId);if(!r)return;
  const inv=window.selChar?.inventory||{};
